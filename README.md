...
- **Time-Series Classification**: MOMENT foundation model (large/small), InceptionTime

### XAI Explainers

The explainer list is **derived per model from pnpxai's architecture detection**
(`detect_model_architecture` via `XaiRecommender`), not a fixed list — so what the UI
offers is what actually runs for that model.
...
```bash
docker exec -w /project <container> python -m backend.scripts.precompute_samples
# default: image + text only — time-series is excluded until its models are trained
# explicit: ... precompute_samples timeseries                 # recompute: --force
```
...
|--------|------|-------------|
| GET | /api/tasks | List supported tasks |
| GET | /api/models?task={task} | List models for task |
| GET | /api/explainers?task={task}&model={model} | List explainers |
| POST | /api/explain | Submit analysis job |
| GET | /api/jobs/{id} | Get job status + results |
| GET | /api/jobs/{id}/visualizations/{name} | Get visualization |
